...
- The library is designed to round-trip existing files _without_ losing information
...
- **Ergonomic to author by hand**  
  Higher-level code should be able to _build_ new files using constructors,
...
- **Not a generic parser framework**  
  You’re designing a _format-specific_ library with a nice API, not a generic
...
### 7.1 Strongly-typed properties _and_ generic `getChildren()`
...
These are _nice-to-haves_, not mandatory. Don’t overdo it: if direct array
...
class VertexPoint extends BaseEntity {
  constructor(public name: string, public point: Ref<CartesianPoint>) {
    /* ... */
  }
}
```
